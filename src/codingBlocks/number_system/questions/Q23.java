...
//We print 12.
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++) {
			totalRickshawCost += Math.min(c1 * rickshaw[i], c2);
		}
		totalRickshawCost = Math.min(totalRickshawCost, c3);

		int totalCabCost = 0;
		for (int i = 0; i < m; i++) {
			totalCabCost += Math.min(c1 * cab[i], c2);
		}
		totalCabCost = Math.min(totalCabCost, c3);

		int finalMinPrice = Math.min(totalRickshawCost + totalCabCost, c4);

		System.out.println(finalMinPrice);
